...
Table 3: CI 플랫폼별 Dagger 연동 설정 비교
| CI 플랫폼 (CI Platform) | 설정 파일 (Configuration File) | 핵심 연동 코드 (Key Integration Snippet) | 참고 / 주요 고려사항 (Notes / Key Considerations) |
|---|---|---|---|
| GitHub Actions | .github/workflows/ci.yml | uses: dagger/dagger-for-github@v5<br>with:<br>  verb: call<br>  args:... | 공식 dagger/dagger-for-github 액션을 사용하여 설정이 매우 간결함. |
| GitLab CI | .gitlab-ci.yml | services:<br>  - docker:dind<br>before_script:<br>  - curl... | sh<br>script:<br>  - dagger call... | docker:dind 서비스가 필요하며, DOCKER_HOST 환경 변수 설정이 중요함. |
| Jenkins | Jenkinsfile | stage('Run Dagger') {<br>  steps {<br>    sh 'curl... | sh'<br>    sh 'dagger call...'<br>  }<br>} | 파이프라인을 실행할 에이전트에 Docker가 설치되어 있어야 함. Credentials 플러그인으로 시크릿 관리. |
이 표는 각 플랫폼의 문법은 다르지만, Dagger를 연동하는 핵심 패턴, 즉 **"CLI 설치 후 dagger call 실행"**은 동일하다는 것을 명확히 보여줍니다. 이것이 바로 Dagger가 제공하는 CI/CD 파이프라인의 이식성입니다.
